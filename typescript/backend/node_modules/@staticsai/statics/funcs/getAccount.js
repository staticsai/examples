 ...
 */
async function getAccount(client, accountId, options) {
    const input = {
        accountId: accountId,
    };
    const parsed = (0, schemas_js_1.safeParse)(input, (value) => operations.GetAccountRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
        return parsed;
    }
    ...
    const path = (0, url_js_1.pathToFunc)("/account/{accountId}")(pathParams);
    ...
    const [result] = await M.match(M.json([200, 302], components.AccountDto$inboundSchema), M.jsonErr(404, errors.GetAccountResponseBody$inboundSchema), M.fail(["4XX", "5XX"]))(response, { extraFields: responseFields });
    if (!result.ok) {
        return result;
    }
    return result;
