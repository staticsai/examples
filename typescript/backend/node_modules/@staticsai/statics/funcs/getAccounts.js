 ...
 */
async function getAccounts(client, options) {
    const path = (0, url_js_1.pathToFunc)("/account")();
    ...
    const [result] = await M.match(M.json([200, 302], components.AccountsDto$inboundSchema), M.fail(["4XX", "5XX"]))(response);
    if (!result.ok) {
        return result;
    }
    return result;
